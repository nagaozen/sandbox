...
Chromium does not start with the aio-computer container (`BROWSER_START_MODE=manual`). Launch it from the desktop's Browser icon; CDP is up a few seconds later. Until then `/v1/capabilities` reports `browser` as `degraded`, and `?refresh=true` re-probes right after launch.
...
docker run --rm -it --security-opt seccomp=unconfined --shm-size 4g \
  -p 127.0.0.1:8091:8091 enterprise-public-cn-beijing.cr.volces.com/vefaas-public/aio-daemon:1.0.1
```

```bash
# Computer image — adds an XFCE desktop and the computer-use worker
docker run --rm -it --security-opt seccomp=unconfined --shm-size 4g \
  -p 127.0.0.1:8091:8091 enterprise-public-cn-beijing.cr.volces.com/vefaas-public/aio-computer:1.0.1
```
