...
| Status | Meaning |
| --- | --- |
| `422` | Unsupported `language`, missing `code`, `timeout` outside 1–900 |
| `422` | A `cwd` that is not a directory |
| `422` on v2, `200` on v1 | An empty `code` string |
| `503` | No interpreter for the language; the response names what it looked for |
| `429` | Session limit reached |
| `404` | Unknown session id on `GET .../sessions/{id}` |
