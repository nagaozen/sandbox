...
`download` is the exception on both sides: it streams bytes on success and answers `404` for a missing path.

Watch routes keep the envelope but use the status for lifecycle failures:

| Status | Meaning |
| --- | --- |
| `404` | An unknown watcher |
| `400` | A `debounce` or `limit` outside its bounds |
| `429` | The watcher limit is reached |
| `503` | A `wait` that timed out |
